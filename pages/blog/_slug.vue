...
<div class="max-w-7xl mx-auto border-gray-700 border-dashed border-l border-r">
        ...
        <div class="space-y-9">
          <div class="py-0.5 border-t border-b border-dashed border-gray-700 flex items-center justify-between">
            <div class="text-yellow-300 font-bold">
              <nuxt-link class="hover:text-yellow-400" :to="localePath('/blog')">{{ $t('blog.header') }}</nuxt-link> <span class="text-gray-700">/</span> {{ post.category }}
            </div>
          ...
          <div class="px-4 sm:px-0 lg:px-4 pb-4 bg-gray-900 rounded-lg">
            ...
            <div class="flex flex-items-center justify-center  rounded-full">
              <UserAvatar :photoURL="post.author.image" :name="post.author.name" class="w-12 h-12 border-2 border-yellow-400 hover:border-hot-pink -mt-5 bg-gray-900 rounded-full"/>
            </div>
            </a>
            <header class="py-4">
              <div class="space-y-1 text-center">
                ...
                <dl class="">
                  <div><dt class="sr-only">Published on</dt>
                    <dd class="text-xs font-medium text-gray-500">
                      <a target="_blank" rel="noreferrer" :href="`https://twitter.com/${post.author.twitter}`"><span class="text-yellow-400 hover:text-indigo-300">{{ post.author.name }}</span></a>
                ...
                <div class="overflow-hidden flex flex-wrap items-center flex-row justify-center">
                    <div class="px-3 text-xs flex flex-shrink-0 py-1 mt-1 mr-2 bg-yellow-400 rounded-md hover:bg-yellow-300" v-for="(tag, index) in post.tags" :key="`tag-${index}`">{{ tag }}</div>
                </div>
              </div>
            </header>
  ...
  <div @click="scrollToTop" class="cursor-pointer fixed z-50 bottom-4 right-4 w-8 w-8 rounded-full bg-gray-900 text-white block text-yellow-400 hover:text-hot-pink">
...
<style scoped>
>>> .icon {
  @apply text-yellow-400 hover:text-hot-pink hidden;
}
