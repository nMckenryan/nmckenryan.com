        ...
        <div class="space-y-9">
          <div class="py-0.5 border-t border-b border-dashed border-gray-700 flex items-center justify-between">
            <div class="text-yellow-300 font-bold">
              <nuxt-link class="hover:text-yellow-400" :to="localePath('/projects')">{{ $t('projects.header') }}</nuxt-link> <span class="text-gray-700">/</span> {{ project.tech }}
            </div>
            ...
            <header class="py-4">
              <div class="space-y-1 text-center">
                ...
                <dl class="">
                  <div><dt class="sr-only">Published on</dt>
                    <dd class="text-xs font-medium text-gray-500">
                      <a target="_blank" rel="noreferrer" :href="project.website"><span class="text-yellow-400 hover:text-indigo-300">{{ project.title }}</span></a>
                      <span class="mx-1">
                        &middot;
                      </span>
                      {{ project.readingTime }}
                ...
                <div class="overflow-hidden flex flex-wrap items-center flex-row justify-center">
                    <div class="px-3 text-xs flex flex-shrink-0 py-1 mt-1 mr-2 bg-yellow-400 rounded-md hover:bg-yellow-300" v-for="(tag, index) in project.tags" :key="`tag-${index}`">{{ tag }}</div>
                </div>
              </div>
            </header>
